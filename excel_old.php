...
PrintHeaders($rowCount,$objPHPExcel,'Imex Information');
$rowCount = $rowCount + 2;
PrintDetail($rowCount,$objPHPExcel,$result,'Shipments Awaiting Export Informaton');
...
//$rowCount = $rowCount + ;
PrintDetail($rowCount,$objPHPExcel,$result,'Shipments With Export Agent (Awaiting Paperwork) ');
$num_rows = mysql_num_rows($result);
//$rowCount = $rowCount + $num_rows + 1;
...
//$rowCount = $rowCount + 2;
PrintDetail($rowCount,$objPHPExcel,$result,'Shipments complete in last 48 hours');
$num_rows = mysql_num_rows($result);
//$rowCount = $rowCount + $num_rows + 1;
...
//		->setValue($header);
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('A'.$startrow, "Status"); 
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$startrow, "Imex Id"); 
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('I'.$startrow, "Sales/Delv No"); 
	$objPHPExcel->getActiveSheet()->SetCellValue('J'.$startrow, "Net Wgt"); 
	$objPHPExcel->getActiveSheet()->SetCellValue('K'.$startrow, "Imex Status"); 
...
Function PrintDetail($startrow,$objPHPExcel,$result,$status)
{
	while($row = mysql_fetch_array($result)){ 			
						
	$objPHPExcel->getActiveSheet()->SetCellValue('A'.$startrow, $status); 
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$startrow, $row[0]); 
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('I'.$startrow, GetOrderNumbers($row[dh_primary_id],$row[dh_header_child],$row[dh_bu],'Y')); 
	$objPHPExcel->getActiveSheet()->SetCellValue('J'.$startrow, GetWeights($row[dh_primary_id],di_net_weight,$row[dh_header_child],$row[dh_import_export]) ); 
	$objPHPExcel->getActiveSheet()->SetCellValue('K'.$startrow, GetStatus($row[5],$row[dh_primary_id],'Y' )); 
