...
header( "Location: dashboard.php?fil=" . $fil . "&live=" . $live); 
...
Function PrintDetail($startrow,$objPHPExcel,$result)
{
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('D'.$startrow, GetBusinessCodes($row[dh_primary_id],$row[dh_header_child]) ); 
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$startrow, GetOrderNumbers($row[dh_primary_id],$row[dh_header_child],$row[dh_bu]) ); 
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('J'.$startrow, GetStatus($row[5],$row[dh_primary_id])  ); 
